...
function examDialog(exam = null) {
  ...
  stepper.innerHTML = ["考试信息", "总分与位置", "本次成绩"].map((label, index) => `<span data-entry-step="${index}">${index + 1}. ${label}</span>`).join("");
...
function publicOverallHistoryCoordinate(exam, share = {}) {
  ...
  return [score, school?.rank != null ? `校内第 ${school.rank} 名` : "", clazz?.rank != null ? `班级第 ${clazz.rank} 名` : ""].filter(Boolean);
}
...
function publicExamDetailV080(exam, share = {}, exams = []) {
  ...
  const overallScoreMetric = overallPrevious ? metricBetween(exam, overallPrevious, null, "auto") : null;
