...
function renderOverview() {
  ...
  const scoreContext = `<div class="current-score-reading ${scoreMetric ? "" : "single"}"><div><span class="section-label">${esc(examScoreLabel(exam,scoreSummary))}</span><strong>${esc(scoreText)}</strong></div>${scoreMetric && shouldShowScoreDelta(scoreMetric) ? `<div class="current-score-change">${renderScoreChange(scoreMetric)}<small>${esc(`和 ${fmtDate(previous.date)} 的总分相比`)}</small></div>` : ""}</div>`;
...
function examDialog(exam = null) {
  ...
  if(!exam){try{const preference=JSON.parse(localStorage.getItem("score-entry-preferences")||"{}");const preferred=preference.subjectsByType?.[form.querySelector("[name=\"type\"]")?.value||"other"];if(Array.isArray(preferred)&&preferred.length)editorScope.subjects=preferred;}catch{}}
  ...
  const entryState=exam?recordCompleteness(exam):null; const editorScope=resolveExamScope(exam); let step=exam&&entryState?.missingSubjects.length?2:exam&&editorScope.isFull&&!entryState?.officialTotal?1:0;
