...
function renderPublicV080(result) {
  ...
  app.innerHTML = `<main class="public-shell ink-share" data-share-view="${view}" data-exam-count="${data.exams?.length || 0}"><header class="share-report-header"><div class="public-brand"><div class="brand-mark">标</div><span>${PRODUCT_NAME} · 分享报告</span><i class="ink-share-flourish" aria-hidden="true"></i></div><div class="share-report-context"><strong>${esc(data.student?.displayName || "学生")}</strong><span>模式：${modeText}</span><span>${futureText}</span></div><div class="privacy-note">此页面由家庭主动分享 · 请勿未经允许转发</div></header>${publicViewNavV080(view, subject)}${body}</main><footer class="footer">分享地址可由家庭随时撤销</footer>`;
}
