...
function directionText(metric) {
  if (!metric) return "还没有可以直接比较的上一场考试";
...
function renderTrajectoryReading(exams = state.exams) {
  ...
  const driverText = driverItems.length
    ...
    : `<p class="muted">还没有足够的相近考试数据来说明变化来源。</p>`;
  return `<section class="reading-section trajectory-reading"><div class="section-head-simple"><div><div class="section-label">多次考试怎么看</div><h2>先看这次，再看前几次有没有同样的变化</h2></div></div><div class="trajectory-reading-grid"><div><small>现在</small><strong>${esc(analysis.current.display)}</strong><span>${esc(trajectoryDirectionLabel(analysis.direction))} · ${esc(trajectoryMetricLabel(analysis.metric))}</span></div><div><small>最近几次</small><strong>${esc(recentText)}</strong><span>${esc(analysis.stability.detail)}</span></div><div><small>从最早一次到现在</small><strong>${esc(longText)}</strong><span>只描述已记录考试，不把一次变化解释成能力变化。</span></div></div><div class="trajectory-driver"><div class="section-label">哪些科目的变化比较明显</div>${driverText}</div>${analysis.skipped.length ? `<small class="muted">另有 ${analysis.skipped.length} 场考试，因为考试条件或可用数据不同，未纳入这次比较。</small>` : ""}</section>`;
}
...
function coordinateRow(exam, className = "coordinate-row") {
  ...
  return `<div class="${className}" aria-label="本场整体位置">${items.map((item) => `<span>${esc(item)}</span>`).join("")}</div>`;
}
...
function subjectMetricValue(exam, key, metric) {
  ...
  if (metric === "auto") {
    ...
    if (clazz?.rank != null) return `班第 ${clazz.rank} 名`;
    return score == null ? null : `${fmtNumber(score)} 分`;
  }
...
function rankingDetails(rankings = []) {
  return rankings.filter((item) => item?.rank != null).map((item) => {
    const scope = item.scope === "school" ? "校内" : item.scope === "class" ? "班级" : item.scope || "范围";
    const participants = item.participants != null ? ` / ${item.participants} 人` : "";
    return `${scope}第 ${item.rank} 名${participants}`;
  }).join(" · ");
}
...
function renderSubjectComparison() {
  ...
  const current = latestExam();
  const currentState = subjectRecordState(current, key);
  const comparison = coreFindComparableExamForSubject(state.exams, current, key, metric);
...
function renderSubjectRows(exam) {
  ...
  return SUBJECTS.map(([key, label]) => {
    ...
    const clazz = subjectRank(exam, key, "class");
    const score = scoreOf(subject);
    const rankParts = [school?.rank != null ? `校第 ${school.rank}` : null, clazz?.rank != null ? `班第 ${clazz.rank}` : null].filter(Boolean).join(" · ");
...
function renderDeepTrajectory() {
  ...
  return `<details class="deep-trajectory" id="deep-trajectory" data-deep-trajectory><summary><span><strong>查看全部记录</strong><small>历次考试与六科历史</small></span><span aria-hidden="true">＋</span></summary><div class="deep-trajectory-body"><p class="trajectory-boundary-note">只有考试条件相近、数据足够的记录会用于直接比较；其他记录仍会保留，方便回看。</p><section><h3>历次考试</h3><div class="history-list">${overallRows}</div></section><section><h3>六科历史</h3><div class="subject-history-list">${subjectHistory}</div></section></div></details>`;
}
...
function renderOverview() {
  ...
  return `<section class="coordinate-hero"><div class="hero-head"><div><h1>${esc(state.student.displayName)}</h1><p>${identityMeta(state.student) || "孩子资料可以稍后补充"}</p></div>${canEdit() ? `<button class="btn btn-outline btn-small" data-action="edit-exam" data-id="${esc(exam.id)}">编辑这次考试</button>` : ""}</div><div class="exam-context"><strong>${esc(exam.name)}</strong><span>${fmtDate(exam.date)} · ${examTypeLabel(exam.type)}</span></div>${coordinateRow(exam)}${completionText ? `<p class="completion-note">${esc(completionText)}</p>` : ""}${schoolPct != null || school?.participants ? `<div class="coordinate-note">${schoolPct != null ? `校前 ${fmtNumber(schoolPct)}%` : ""}${schoolPct != null && school?.participants ? " · " : ""}${school?.participants ? `本次共 ${school.participants} 人` : ""}</div>` : ""}</section><section class="reading-section change-section"><div class="section-label">和上一场可以直接比较的考试相比</div><div class="change-main"><strong>${esc(directionText(overallMetric))}</strong><span>${esc(overallMetric?.detail || comparisonNote)}</span><p class="human-summary">${esc(humanChangeSummary(overallMetric))}</p></div>${previous ? `<small>比较对象：${comparisonNote}</small>` : `<small>${comparisonNote}</small>`}</section><section class="reading-section"><div class="section-head-simple"><div><div class="section-label">值得回看的科目</div><h2>哪些科目的变化比较明显</h2></div></div>${sources.length ? `<div class="change-source-list">${sources.map(({ key, label, metric }) => { const current = subjectRank(exam, key, "school") || subjectRank(exam, key, "class"); return `<div class="change-source-row"><strong>${label}</strong><span>${esc(metric.detail)}</span><small>${esc(metric.metric === "schoolRank" ? "以学校排名为依据" : metric.metric === "classRank" ? "以班级排名为依据" : "以分数为依据")}</small></div>`; }).join("")}</div>` : `<p class="muted">还没有足够的相近考试数据。历史仍会保留，数据够用时再比较。</p>`}</section><section class="reading-section subjects-section"><div class="section-head-simple"><div><div class="section-label">六科</div><h2>这次考试的六科成绩</h2></div></div><div class="subject-rows">${renderSubjectRows(exam)}</div></section>${canEdit() ? `<section class="overview-actions" aria-label="下一步"><button class="btn btn-primary btn-block" data-action="${primaryAction}" data-id="${esc(exam.id)}" data-primary-action="record-next">${primaryLabel}</button><button class="btn btn-outline" data-action="open-trajectory" aria-controls="deep-trajectory">查看全部记录</button></section>` : ""}${renderTrajectoryReading(state.exams)}${renderDeepTrajectory()}`;
}
...
function renderDashboard() {
  ...
  app.innerHTML = `${renderHeader()}<main class="container"><nav class="tabs" aria-label="主导航"><button class="tab ${state.tab === "overview" ? "active" : ""}" data-tab="overview">成绩</button><button class="tab ${state.tab === "exams" ? "active" : ""}" data-tab="exams">考试</button><button class="tab ${state.tab === "sharing" ? "active" : ""}" data-tab="sharing">分享</button><button class="tab ${state.tab === "family" ? "active" : ""}" data-tab="family">家庭</button></nav><div class="status-region is-${state.noticeTone}" data-status-region role="status" aria-live="polite" ${state.notice ? "" : "hidden"}>${esc(state.notice)}</div>${body}</main><footer class="footer">${PRODUCT_NAME} · 数据默认只对家庭成员可见 · ${esc(state.me?.appVersion || "")}</footer>`;
  bindDashboard();
}
...
function publicSubjectRows(exam, share = {}) {
  ...
  return SUBJECTS.map(([key, label]) => {
    ...
    const meta = [school?.rank != null ? `校第 ${school.rank}` : null, clazz?.rank != null ? `班第 ${clazz.rank}` : null].filter(Boolean).join(" · ");
...
function publicBaselineV081(view, exams, share = {}) {
  ...
  const future = share.includesFutureExams ? "以后新增的考试也会显示在这里；当前页面只显示你选择分享的内容。" : "这是创建分享时固定的内容；以后新增考试不会加入。";
  return `<aside class="public-baseline-note" aria-label="记录状态"><span class="public-baseline-mark" aria-hidden="true"></span><div><strong>目前的记录</strong><p>${copy[view] || copy.total}</p><p>${future}</p></div></aside>`;
}
...
function publicTrajectoryReading(exams, key = null, share = {}) {
  ...
  const comparisonBoundary = share.fields?.comparisonContext === true
    ? "只按已分享的考试比较"
    : "没有分享比较范围，只按考试类别参考";
  return `<aside class="public-analysis-note"><div class="section-label">${key ? "这门课怎么看" : "多次考试怎么看"}</div><div class="public-analysis-grid"><div><small>现在</small><strong>${esc(analysis.current.display)}</strong><span>${esc(trajectoryDirectionLabel(analysis.direction))} · ${esc(metricLabel)}</span></div><div><small>最近几次</small><strong>${esc(analysis.stability.detail)}</strong><span>${esc(analysis.stability.detail)}</span></div><div><small>长期</small><strong>${esc(analysis.longDirection === "forward" ? "相对第一次向前" : analysis.longDirection === "backward" ? "相对第一次向后" : "与第一次接近")}</strong><span>${esc(comparisonBoundary)}</span></div></div>${analysis.skipped.length ? `<small>有 ${analysis.skipped.length} 场记录未纳入，因为考试条件或数据不足。</small>` : ""}</aside>`;
}
...
function publicExamDetailV080(exam, share = {}) {
  ...
  const rows = SUBJECTS.map(([key, label]) => {
    ...
    const school = rankShared ? rankByScope(subject.rankings, "school") : null;
    const clazz = rankShared ? rankByScope(subject.rankings, "class") : null;
    const values = [score == null ? null : `${fmtNumber(score)} 分`, subject.fullScore ? `满分 ${subject.fullScore}` : null, school?.rank != null ? `校第 ${school.rank}` : null, clazz?.rank != null ? `班第 ${clazz.rank}` : null].filter(Boolean).join(" · ");
