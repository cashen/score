...
function rankText(ranking) {
  if (!ranking) return "—";
  if (ranking.rank != null && ranking.participants != null) return `${ranking.rank} / ${ranking.participants}`;
  if (ranking.rank != null) return `第 ${ranking.rank} 名`;
  if (ranking.participants != null) return `共 ${ranking.participants} 人`;
  return "—";
}

function rankHint(ranking) {
  if (!ranking) return "尚无排名";
  const parts = [];
  const pct = percentile(ranking.rank, ranking.participants);
  if (pct != null) parts.push(`前 ${pct}%`);
  else if (ranking.rank != null && ranking.participants == null) parts.push("总人数未填");
  else if (ranking.rank == null && ranking.participants != null) parts.push("名次未填");
  if (ranking.label) parts.push(esc(ranking.label));
  return parts.join(" · ") || "排名信息待补";
}
...
function rankCard(label, ranking) {
  ...
  return `<div class="rank-card"><small>${label}</small><strong>${rankText(ranking)}</strong><small>${rankHint(ranking)}</small></div>`;
}
...
function trendHtml() {
  const points = state.exams.slice(0, 6).reverse().map((exam) => {
    ...
    return { exam, ranking, pct };
  });
  const validPct = points.filter((p) => p.pct != null);
  if (validPct.length) {
    const max = Math.max(...validPct.map((p) => p.pct), 1);
  ...
  const validRank = points.filter((p) => Number.isInteger(p.ranking?.rank));
  if (!validRank.length) return `<div class="empty">还没有足够的学校排名数据形成趋势。</div>`;
  const maxRank = Math.max(...validRank.map((p) => p.ranking.rank), 1);
  return `<div class="muted" style="font-size:12px;margin-top:8px">总人数未填时，仅显示校排名名次变化。</div><div class="trend-strip">${points.map(({ exam, ranking }) => {
    const rank = ranking?.rank;
    const height = rank == null ? 5 : Math.max(8, Math.round((1 - Math.min(rank, maxRank) / (maxRank + 1)) * 58));
    return `<div class="trend-point"><div class="trend-bar-wrap"><div class="trend-bar" style="height:${height}px;opacity:${rank == null ? .18 : .82}"></div></div><strong>${rank == null ? "—" : `#${rank}`}</strong><span>${esc(exam.name)}</span></div>`;
  }).join("")}</div>`;
}
...
function subjectCards(exam) {
  ...
  return SUBJECTS.map(([key, label]) => {
    ...
    return `<div class="subject-card"><div class="eyebrow">${label}${converted ? " · 赋分" : ""}</div><div class="score">${score ?? "—"}</div><div class="meta">校 ${rankText(school)}<br>班 ${rankText(clazz)}</div></div>`;
  }).join("");
}
...
function shareFieldsControls(prefix, defaults = {}) {
  const fields = [
    ...
    ["overallScore", "总分"], ["overallRank", "总排名"], ["subjectScores", "单科成绩"], ["subjectRanks", "单科排名"], ["history", "历次考试对比"]
  ];
...
function renderSharing() {
  return `<section><div class="section-head"><div><h2>分享与隐私</h2><div class="muted">默认仅家庭成员可见。分享输出由服务端白名单生成，备注不会对外返回。</div></div></div>${state.notice ? `<div class="notice-box">${esc(state.notice)}</div>` : ""}<div class="share-grid"><article class="card share-card"><h3>私密分享链接</h3><p class="muted">拿到随机链接的人可以查看。勾选“历次考试对比”后，会一起展示多次考试的变化。</p><div class="field"><label>分享模式</label><select id="secret-mode"><option value="live">实时：以后新增成绩也可见</option><option value="snapshot">快照：只保留创建时的数据</option></select></div><div class="field"><label>有效期（可不填）</label><input id="secret-expiry" type="date"></div>${shareFieldsControls("secret", { school: false, className: false })}<button class="btn btn-primary" data-action="create-secret">生成私密链接</button></article><article class="card share-card"><h3>公开主页</h3><p class="muted">无需登录即可访问，但默认 noindex，不创建学生搜索或排行榜。</p><div class="field"><label>公开地址</label><div style="display:flex;gap:8px;align-items:center"><span class="muted">/p/</span><input id="public-slug" minlength="3" maxlength="50" autocapitalize="none" spellcheck="false" placeholder="例如 abc"></div><small class="muted">3–50 位字母、数字或短横线；大写会自动转小写。</small></div><div class="field"><label>分享模式</label><select id="public-mode"><option value="live">实时主页</option><option value="snapshot">固定快照</option></select></div>${shareFieldsControls("public", { school: false, className: false })}<button class="btn btn-primary" data-action="create-public">创建公开主页</button></article></div><div class="section-head"><h2>当前分享</h2></div><div class="card card-pad">${state.shares.length ? state.shares.map((item) => `<div class="share-item"><div><strong>${item.kind === "secret" ? "私密链接" : "公开主页"}</strong> <span class="badge">${item.mode === "snapshot" ? "快照" : "实时"}</span>${item.expiresAt ? ` <span class="badge badge-warn">到期 ${esc(item.expiresAt.slice(0,10))}</span>` : ""}<div class="muted" style="margin-top:5px">创建于 ${esc(item.createdAt.slice(0,10))}${item.kind === "secret" ? " · 出于安全原因，旧私密 token 不回显；需要重新分享时可撤销后再生成。" : ` · /p/${esc(item.locator)}`}</div></div><button class="btn btn-danger btn-small" data-action="revoke-share" data-kind="${item.kind}" data-locator="${esc(item.locator)}">撤销</button></div>`).join("") : `<div class="empty">当前没有任何外部分享，孩子成绩仅家庭内可见。</div>`}</div></section>`;
}
...
function rankInputs(prefix, ranking) {
  return `<input name="${prefix}-rank" inputmode="numeric" placeholder="排名" value="${ranking?.rank ?? ""}"><input name="${prefix}-participants" inputmode="numeric" placeholder="人数（可空）" value="${ranking?.participants ?? ""}">`;
}
...
function publicSubjectCards(exam) {
  ...
  return SUBJECTS.map(([key, label]) => {
    ...
    const school = rankByScope(subject.rankings, "school");
    return `<div class="subject-card"><div class="eyebrow">${label}</div><div class="score">${score ?? "—"}</div><div class="meta">校 ${rankText(school)}</div></div>`;
  }).join("");
}

function sharedComparisonHtml(exams) {
  if (!Array.isArray(exams) || exams.length < 2) return "";
  const chronological = [...exams].reverse();
  const schoolPoints = chronological.map((exam) => {
    const ranking = rankByScope(exam.overallRankings, "school");
    const pct = ranking ? percentile(ranking.rank, ranking.participants) : null;
    return { exam, ranking, pct };
  });
  const hasSchoolPosition = schoolPoints.some(({ ranking }) => ranking?.rank != null);
  const schoolPosition = hasSchoolPosition ? `<section class="card card-pad share-position-card"><div class="share-compare-title"><div><div class="eyebrow">学校相对位置</div><h3>从较早到最近</h3></div><span class="badge">${chronological.length} 次考试</span></div><div class="share-position-track">${schoolPoints.map(({ exam, ranking, pct }) => `<div class="share-position-point"><small>${fmtDate(exam.date)}</small><strong>${pct != null ? `前 ${pct}%` : ranking?.rank != null ? `第 ${ranking.rank} 名` : "—"}</strong><span>${pct != null ? rankText(ranking) : ranking?.rank != null ? "总人数未填" : "未提供校排名"}</span><em>${esc(exam.name)}</em></div>`).join("")}</div><p class="share-compare-note">有总人数时显示学校百分位；没有总人数时只比较名次，不强行推算百分位。</p></section>` : "";

  const examCards = `<div class="share-exam-compare-grid">${chronological.map((exam, index) => {
    const school = rankByScope(exam.overallRankings, "school");
    const clazz = rankByScope(exam.overallRankings, "class");
    return `<article class="card share-exam-compare ${index === chronological.length - 1 ? "is-latest" : ""}"><div class="share-exam-head"><div><strong>${esc(exam.name)}</strong><small>${fmtDate(exam.date)}</small></div>${index === chronological.length - 1 ? `<span class="badge">最近</span>` : ""}</div><div class="share-exam-metrics"><div><small>总分</small><strong>${exam.overallScore ?? "—"}</strong></div><div><small>校排名</small><strong>${rankText(school)}</strong></div><div><small>班排名</small><strong>${rankText(clazz)}</strong></div></div></article>`;
  }).join("")}</div>`;

  const subjectRows = SUBJECTS.map(([key, label]) => {
    const points = chronological.map((exam) => ({ exam, score: scoreOf(exam.subjects?.[key]) }));
    if (points.filter((point) => point.score != null).length < 2) return "";
    return `<div class="share-subject-trend"><strong>${label}</strong><div class="share-subject-points">${points.map(({ exam, score }) => `<span><small>${fmtDate(exam.date)}</small><b>${score ?? "—"}</b></span>`).join("")}</div></div>`;
  }).filter(Boolean).join("");

  return `<section class="share-comparison-section"><div class="section-head share-comparison-head"><div><h2>历次考试对比</h2><div class="muted">不同考试难度可能不同，所以优先看学校相对位置；分数用于辅助观察。</div></div><span class="badge">共 ${chronological.length} 次</span></div>${schoolPosition}${examCards}${subjectRows ? `<section class="card card-pad share-subject-comparison"><div class="eyebrow">单科分数轨迹</div><h3>同一科目放在一起看</h3><div class="share-subject-trends">${subjectRows}</div></section>` : ""}</section>`;
}
