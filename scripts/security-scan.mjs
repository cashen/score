...
const sensitiveAssignment = /(?:CLOUDFLARE_API_TOKEN|SCORE_(?:SESSION_SECRET|AUTH_PEPPER|ADMIN_BOOTSTRAP_SECRET|TOKEN_PEPPER))\s*[:=]\s*(?!\$\{\{\s*secrets\.)["']?[A-Za-z0-9_+/=.-]{16,}/;
const history = git(["log", "--all", "--format=%H", "-G", sensitiveAssignment.source, "--"]).split("\n").filter(Boolean);
