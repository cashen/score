...
export function normalizeSubjectSet(value){if(!Array.isArray(value))return null;if(value.some((key)=>!valid(key)))throw Object.assign(new Error("考试科目无效"),{code:"invalid_subject_set",field:"subjectSet"});const result=[...new Set(value)];if(!result.length)throw Object.assign(new Error("本次考试至少选择一门科目"),{code:"empty_subject_set",field:"subjectSet"});return result;}
export function legacySubjectSet(){return [...SUBJECT_KEYS];}
export function resolveExamScope(exam){const explicit=Array.isArray(exam?.subjectSet)?exam.subjectSet.filter(valid):null;const subjects=explicit?.length?[...new Set(explicit)]:legacySubjectSet();return{subjects,source:explicit?.length?"explicit":"legacy-fallback",subjectCount:subjects.length,isSingle:subjects.length===1,isFull:subjects.length===SUBJECT_KEYS.length};}
