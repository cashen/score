...
function passwordIterations(env) {
  return Math.max(10000, Math.min(100000, Number(env.PASSWORD_ITERATIONS) || 100000));
}
...
async function handleRecoveryLinkReset(request, env, rawToken) {
  ...
  try {
    ...
    const consumed = await consumeOneTime(env, "recovery-link", found.locator, claimId);
    if (!consumed) throw Object.assign(new Error("重置链接状态无法确认，请重新获取链接"), { status: 500, code: "recovery_link_consume_failed" });
    return json({ ok: true, recoveryCode: rotatedRaw, notice: "密码已重置，旧会话和这条重置链接均已失效。新的恢复码只显示这一次。" });
  } catch (error) {
    await releaseOneTime(env, "recovery-link", found.locator, claimId);
    throw error;
  }
}
