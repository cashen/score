...
const coordinateInsight = await readFile(new URL("../public/coordinate-insight-v100.js", import.meta.url), "utf8");
...
test("coordinate insight observes only the stable app mount point", () => {
  assert.match(coordinateInsight, /const appRoot = document\.querySelector\("#app"\)/);
  assert.match(coordinateInsight, /observer\.observe\(appRoot, \{ childList: true \}\)/);
  assert.doesNotMatch(coordinateInsight, /observer\.observe\(document\.body, \{ childList: true, subtree: true \}\)/);
});
