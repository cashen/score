...
test("verifyProduction adds Cloudflare Worker version override to every request", async () => {
  ...
  const fetchFn = async (url, options = {}) => {
    ...
    if (path === "/api/health") {
      return new Response(JSON.stringify({ ok:true, appVersion:"0.13.5.2", buildSha, schemaVersion:1, storage:"workers-kv" }), {status:200,headers:{"content-type":"application/json"}});
    }
  ...
  await verifyProduction({origin:"https://score-track.cashen.workers.dev",buildSha,version:"0.13.5.2",versionId,publicRoot,fetchFn});
