...
test("legacy inference only claims a single subject when safe",()=>{assert.deepEqual(resolveExamScope({subjects:{english:{rawScore:100}}}).subjects,["english"]);assert.equal(resolveExamScope({subjects:{english:{rawScore:100},physics:{rawScore:80}}}).isFull,true);});
